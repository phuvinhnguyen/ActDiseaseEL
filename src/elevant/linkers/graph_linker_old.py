...
@cache
def parse_obo_file(obo_path = '/media/volume/LLMRag2/.local/HumanDiseaseOntology/src/ontology/doid-merged.obo'):
...
def extract_spans_with_entities(text: str, entities_matcher: Dict, top_k: int = 10, 
    ...
    for n in range(min_span_length, max_span_length + 1):
        for i in range(len(words) - n + 1):
            ...
            end_char = word_positions[i + n - 1][1]
            ...
            span_text_for_matching = ' '.join(actual_words)
            ...
            if span_key not in spans_info:
                ...
                if near_entities:
                    spans_info[span_key] = {
                        'span_text': span_text_for_matching,
