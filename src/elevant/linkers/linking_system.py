...
class LinkingSystem:
    ...
    def _initialize_linker(self, linker_name: str, prediction_file: str, prediction_format: str, api_url: str):
        ...
        elif linker_type == Linkers.BM25.value:
            from elevant.linkers.bm25_linker import BM25Linker
            # Online BM25 linker - no database required
            self.linker = BM25Linker(self.linker_config)
        elif linker_type == Linkers.BM25_DB.value:
            from elevant.linkers.bm25_db_linker import BM25DatabaseLinker
            # BM25 with local database - load necessary mappings
            self.load_missing_mappings({MappingName.WIKIDATA_ALIASES,
                                        MappingName.FAMILY_NAME_ALIASES,
                                        MappingName.HYPERLINK_TO_MOST_POPULAR_CANDIDATES,
                                        MappingName.NAME_TO_ENTITY_ID,
                                        MappingName.SITELINKS})
            self.linker = BM25DatabaseLinker(self.entity_db, self.linker_config)
        elif linker_type == Linkers.GRAPH_LLM.value:
        ...
        elif linker_type == Linkers.RANKING_LLM.value:
            from elevant.linkers.ranking_linker import RankingLinker
            self.load_missing_mappings({MappingName.WIKIDATA_ALIASES,
                                        MappingName.FAMILY_NAME_ALIASES,
                                        MappingName.NAME_TO_ENTITY_ID,
                                        MappingName.SITELINKS})
            self.linker = RankingLinker(self.entity_db, self.linker_config)
        elif linker_type == Linkers.ONENET_LLM.value:
